...
function loadState(): StoreState {
  ...
  try {
    ...
    // Discard any previously seeded fake candidates (ids starting with cand_001..cand_024)
    const storedCandidates = (Array.isArray(parsed.candidates) ? parsed.candidates as CandidateProfile[] : []).filter(
      (c) => !/^cand_0[0-2][0-9]$/.test(String(c.id))
    );
    // Discard the hardcoded default job
    const storedJobs = (Array.isArray(parsed.jobs) ? parsed.jobs as Job[] : []).filter(
      (j) => j.id !== 'job-senior-backend-001'
    );
    return {
      ...fallback,
      ...parsed,
      jobs: storedJobs,
      activeJobId: storedJobs[0]?.id || '',
